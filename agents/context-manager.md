...
- **Default scope is global.** The shared context store lives at `~/.claude/context/` (on this machine, `C:/Users/PC/.claude/context/`) and is shared across every project. Use it unless the user explicitly asks for project-local context, in which case use `<project-root>/.claude/context/` instead. Never silently split context between the two — if both exist, say so and ask which is authoritative.
...
- Which agents will share the context. Scope defaults to global (`~/.claude/context/`); a project root is only needed if the user asks for project-local context.
...
- **Design the layout.** Decide the directory and file structure for shared context (e.g. the global `~/.claude/context/` with `state.md`, `decisions.md`, `task-history.md`).
