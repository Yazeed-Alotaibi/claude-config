...
```bash
agy --print "$(cat "$PROMPT_FILE")" --model gemini-3.1-pro-high --sandbox
rm -f "$PROMPT_FILE"
```
...
- Strongest available models are used: Opus for Reviewer A, GPT-5.4 (Codex) or Gemini 3.1 Pro (Antigravity) for Reviewer B.
- External reviewers run sandboxed (`--sandbox read-only` for Codex, `--sandbox` for Antigravity) to prevent repo mutation during review.
